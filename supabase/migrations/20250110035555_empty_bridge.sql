/*
  # Fix problem sets foreign key relationship

  1. Changes
    - Keep only the foreign key to auth.users
    - Update the join in RLS policies
    - Add index for performance

  2. Security
    - Maintain existing RLS policies but update them to use proper joins
*/
...
-- Add back the correct foreign key constraint to auth.users
ALTER TABLE public.problem_sets
